...
module.exports = async () => {
  ...
  for (let i = 0; i < count - 1; i += count2) { //50000000
    ...
    for (let b = i; b < i + count2; b++) {
      ...
      const hash = md5(`${name}0${i}@${domains[0]}`);
      ...
      output += `('{}',true,10,${base_id},'name_${name}0${i}@${domains[0]}','${email}',decode('${hash}', 'hex'),decode('${hash}', 'hex'),'${domains[0]}','${lead_active}',${gender},${year_of_birth},${city},${province},'2018-12-12 22:34:15','surname_${name}000000000${i}@${domains[0]}'),\n`;
    }
  ...
  const base_id = 1;
  const name = randomString(20);
  const hash = md5(`${name}00@${domains[0]}`);
  ...
  const city = Math.floor(Math.random() * 85);
  const province = Math.floor(Math.random() * 16);
  const lastOutput = `('{}',true,10,${base_id},'name_${name}00@${domains[0]}','${email}',decode('${hash}', 'hex'),decode('${hash}', 'hex'),'${domains[0]}','${lead_active}',${gender},${year_of_birth},${city},${province},'2018-12-12 22:34:15','surname_${name}0000000000@${domains[0]}');`;
